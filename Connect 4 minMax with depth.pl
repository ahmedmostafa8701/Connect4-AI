...
minMax(Board, Colomn, Player):-
    minMaxHelper(Board, Player, 0, null, null, _, Colomn, 0).
minMaxHelper(_, _, Index, Score, Col, Score, Col, _):-
    Index > 6, !.
minMaxHelper(Board, Player, Index, PScore, PCol, Score, Colomn, D):-
    addPiece(Board, Index, Added, Player, _),
    Index2 is Index + 1,
    Added = Board -> (minMaxHelper(Board, Player, Index2, PScore, PCol, Score, Colomn, D)),!.
minMaxHelper(Board, Player, Index, PScore, PCol, Score, Colomn, D):-
    addPiece(Board, Index, Added, Player, _),
    ...
    D2 is D + 1,
    ((D2 < 2) ->
    (minMaxHelper(Added, Player2, 0, null, null, ScoreOp, _, D2),
     (ScoreOp \= null -> (Score2 is (ScoreOp * -1), !); Score2 = null), !
     ); Score2 = null),
    best(PScore, PCol, Score2, Index, Score3, Col3),
    minMaxHelper(Board, Player, Index2, Score3, Col3, Score, Colomn, D),!.
minMaxHelper(Board, Player, Index, PScore, PCol, Score, Colomn, D):-
    addPiece(Board, Index, Added, Player, _),
    measureState(Added, Score1, Player),
    Score1 \= unKnown,
    ...
    Index2 is Index + 1,
    (Score1 \= 1 -> (minMaxHelper(Board, Player, Index2, Score3, Col3, Score, Colomn, D), !);(Score = 1, Colomn = Index)).
